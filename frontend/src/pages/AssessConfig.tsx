...
const AssessConfig: React.FC<AssessConfigProps> = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.post("/api/topics/", config)
      ...
      <div className="min-h-screen pt-20 px-4 relative z-10">
        ...
        >
          <Card className="p-8">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => handleInputChange("qnCount", Number.parseInt(e.target.value))}
                      disabled={isSubmitting}
                      className="w-full h-2 bg-muted/30 rounded-lg appearance-none cursor-pointer"
                    />
                    <div className="flex justify-between text-sm text-muted-foreground mt-2">
                      <span>1</span>
                      <span className="text-foreground font-semibold">{config.qnCount} questions</span>
                      <span>50</span>
                    </div>
                  </div>
                </div>
            ...
            >
              <h3 className="text-lg font-semibold text-foreground mb-2">Assessment Preview</h3>
              <div className="text-sm text-muted-foreground space-y-1">
                <p>
                  <span className="font-medium">Topic:</span> {config.topic || "Not specified"}
                </p>
                <p>
                  <span className="font-medium">Questions:</span> {config.qnCount}
                </p>
                <p>
                  <span className="font-medium">Difficulty:</span> {config.difficulty}
                </p>
                <p>
                  <span className="font-medium">Estimated Time:</span> {Math.ceil(config.qnCount * 1.5)} minutes
                </p>
              </div>
            </motion.div>
          </Card>
